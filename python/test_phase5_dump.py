...
NOTA: questo test NON avvia GStreamer. Serve solo a verificare che
  onMediaWithTimestampIndication riceva dati validi prima di collegare
  il decoder hardware (Step 9).
"""
...
def main():
    ...
    # Override del placeholder Phase 5: abilita dump su file
    # tramite il binding C++ enable_video_dump(path)
    gst_sink = None
    if hasattr(core, "GstVideoSink"):
        gst_sink = core.GstVideoSink(800, 480)
        # WId = 0 -> autovideosink standalone (apre una finestra di test)
        gst_sink.set_window_id(0)

    def _on_video_channel_open(payload: bytes) -> bytes:
        print("\n[Test5a] *** Video channel aperto ***")
        if gst_sink:
            print("[Test5a] Avvio GStreamer con autovideosink (WId=0)...")
            gst_sink.start_pipeline()
        else:
            print("[Test5a] GstVideoSink non disponibile (build senza GStreamer).")
            print("[Test5a] I NAL units saranno solo loggati a console.")
        return b""
