"""
Strict Python implementation for RadioEventHandler.
"""
...
class RadioEventHandlerLogic:
    ...
    def on_channel_open_request(self, handler, payload):
        ...
        if res_bytes:
            ...
            channel.send_channel_open_response(resp, strand, "Radio/ChannelOpenResponse")
            _logger.info("Radio: ChannelOpenResponse inviata (canale aperto)")
        channel.receive(handler)

    def on_channel_error(self, handler, payload):
        _logger.error("Radio: ChannelError %s", payload)
        self._orchestrator.on_channel_error("Radio", payload)
