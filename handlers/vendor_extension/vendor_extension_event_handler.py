"""
Strict Python implementation for VendorExtensionEventHandler.
"""
...
_logger = get_logger("app.vendor_extension")


class VendorExtensionOrchestrator:
    def __init__(self):
...
class VendorExtensionEventHandlerLogic:
    ...
    def on_channel_open_request(self, handler, payload):
        ...
        if res_bytes:
            ...
            channel.send_channel_open_response(resp, strand, "VendorExtension/ChannelOpenResponse")
            _logger.info("VendorExtension: ChannelOpenResponse inviata (canale aperto)")
        channel.receive(handler)

    def on_channel_error(self, handler, payload):
        _logger.error("VendorExtension: ChannelError %s", payload)
        self._orchestrator.on_channel_error("VendorExtension", payload)
