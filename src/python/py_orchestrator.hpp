...
public:
    ...
    std::string onAvChannelSetupRequest(int channel_id, const std::string& request_bytes) override {
        return callPythonMethodWithChannel("on_av_channel_setup_request", channel_id, request_bytes);
    }

    // Step 2: ChannelOpenRequest
    // Chiama Python: on_channel_open_request(channel_id: int, payload: bytes) -> bytes
    std::string onChannelOpenRequest(int channel_id, const std::string& request_bytes) override {
        return callPythonMethodWithChannel("on_channel_open_request", channel_id, request_bytes);
    }
