...
public:
    ...
    void onMediaChannelSetupRequest(
        ...
        std::cout << "[Audio CH" << channel_id_ << "] AVChannelSetupRequest received." << std::endl;
        if (!orchestrator_) {
        ...
        if (!config_resp.ParseFromString(res_str)) {
            std::cerr << "[Audio CH" << channel_id_ << "] Config parse FAILED" << std::endl;
            channel_->receive(this->shared_from_this());
            return;
        }
        channel_->sendChannelSetupResponse(config_resp,
            makePromise(("Audio/AVChannelSetupResponse_CH" + std::to_string(channel_id_)).c_str()));
        channel_->receive(this->shared_from_this());
    }
    ...
    void onChannelOpenRequest(
        ...
        std::cout << "[Audio CH" << channel_id_ << "] ChannelOpenRequest received." << std::endl;
        if (!orchestrator_) {
            channel_->receive(this->shared_from_this());
            return;
        }
        ...
        channel_->sendChannelOpenResponse(open_resp,
            makePromise(("Audio/ChannelOpenResponse_CH" + std::to_string(channel_id_)).c_str()));
        channel_->receive(this->shared_from_this());
    }
    ...
    void onMediaChannelStartIndication(
        ...
        std::cout << "[Audio CH" << channel_id_ << "] MediaChannelStart session="
    ...
    void onMediaChannelStopIndication(
        ...
        std::cout << "[Audio CH" << channel_id_ << "] MediaChannelStop" << std::endl;
    ...
    void onMediaWithTimestampIndication(
        ...
        channel_->sendMediaAckIndication(ack,
            makePromise(("Audio/MediaAck_CH" + std::to_string(channel_id_)).c_str()));
        channel_->receive(this->shared_from_this());
    }
    ...
    void onChannelError(const aasdk::error::Error& e) override {
        std::cerr << "[Audio CH" << channel_id_ << "] Channel Error: " << e.what() << std::endl;
    }
